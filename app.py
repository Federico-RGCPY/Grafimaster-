...
# =============================================================================
# MÓDULO 4: COBRANZAS Y ANULACIONES (ACTUALIZADO)
# =============================================================================
elif menu == "💵 Cobranzas y Anulaciones":
    st.header("💵 Gestión de Cobranzas y Anulación de Asientos")

    pestana_cobro, pestana_anulacion = st.tabs(["💳 Registrar Cobro", "🚫 Anular Pago / Asiento Registrado"])

    # -------------------------------------------------------------------------
    # TAB 1: REGISTRAR COBRO (EFECTIVO O BANCO)
    # -------------------------------------------------------------------------
    with pestana_cobro:
        if not st.session_state.facturas.empty:
            ...
            if facturas_pendientes.empty:
                ...
                with st.form("form_cobro"):
                    ...
                    if st.form_submit_button("💾 Registrar Cobro"):
                        ...
                        # Si el cobro fue vía Banco Itaú, impacta en la tabla de Banco
                        ...
                        guardar_tabla(st.session_state.facturas, "Facturas")
                        st.success(f"✅ Cobro de {formatear_pyg(monto_a_cobrar)} registrado correctamente por {metodo_pago}.")
                        st.rerun()
        else:
            st.info("No existen facturas registradas.")

    # -------------------------------------------------------------------------
    # TAB 2: ANULAR PAGO / ASIENTO
    # -------------------------------------------------------------------------
    with pestana_anulacion:
        st.subheader("🚫 Revertir o Anular un Cobro Realizado")
        st.caption("Usa esta opción si te equivocaste al registrar un cobro para restablecer el saldo pendiente de la factura.")

        # Filtrar facturas que tengan algún pago cargado
        facturas_cobradas = st.session_state.facturas[st.session_state.facturas["Monto_Pagado"] > 0]

        if facturas_cobradas.empty:
            st.info("No hay pagos cargados actualmente para anular.")
        else:
            opciones_anular = {}
            for _, r in facturas_cobradas.iterrows():
                k_text = f"Factura #{r['ID']} - {r['Cliente']} [{r['Trabajo']}] (Monto Pagado a Revertir: {formatear_pyg(r['Monto_Pagado'])})"
                opciones_anular[k_text] = int(r['ID'])

            factura_anular_sel = st.selectbox("Seleccione la Factura para Anular/Resetear su pago:", list(opciones_anular.keys()))
            id_fact_anular = opciones_anular[factura_anular_sel]

            factura_obj = st.session_state.facturas[st.session_state.facturas["ID"] == id_fact_anular].iloc[0]

            st.warning(f"⚠️ **Atención:** Se eliminarán los cobros asociados a la Factura #{id_fact_anular} por {formatear_pyg(factura_obj['Monto_Pagado'])}, restableciendo su estado a **Pendiente**.")

            if st.button("🔴 Confirmar Anulación del Pago"):
                idx_f = st.session_state.facturas[st.session_state.facturas["ID"] == id_fact_anular].index[0]
                
                # 1. Resetear monto pagado y estado en Facturas
                st.session_state.facturas.at[idx_f, "Monto_Pagado"] = 0.0
                st.session_state.facturas.at[idx_f, "Estado"] = "Pendiente"

                # 2. Eliminar asientos asociados del extracto de Banco Itaú si los hubiere
                if not st.session_state.banco.empty:
                    # Filtrar eliminando el asiento vinculado a esta Factura_ID
                    st.session_state.banco = st.session_state.banco[st.session_state.banco["Factura_ID"] != id_fact_anular]
                    guardar_tabla(st.session_state.banco, "Banco_Itau")

                guardar_tabla(st.session_state.facturas, "Facturas")
                st.success(f"✅ Se anuló el pago de la Factura #{id_fact_anular} con éxito. El saldo volvió a Pendiente.")
                st.rerun()
